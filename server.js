...
app.post('/api', (req, res) => {
  ...
  try {
    switch (action) {
      ...
      case 'forfeit': return res.json(actionForfeit(req.body));
...
function actionCreate(body) {
  ...
  games[gameId] = {
    ...
    status: 'waiting',
    players: {
      south: { token, name, ping: Date.now(), connected: true },
      north: null
    },
    ...
    gameStatus: 'playing',
    winner: null,
    forfeit: null,   // side qui a abandonné
    lastMove: null,
    chat: [],
    created: Date.now()
  };
...
function actionJoin(body) {
  ...
  const token = crypto.randomBytes(16).toString('hex');
  game.players.north = { token, name, ping: Date.now(), connected: true };
  game.status = 'playing';
...
function actionPoll(body) {
  ...
  // Vérifier abandon par timeout (15 secondes sans ping)
  checkAbandonment(game);
...
function actionPing(body) {
  ...
  const side   = sideFromToken(game, token);
  if (side) {
    game.players[side].ping      = Date.now();
    game.players[side].connected = true;
  }
  return { ok: true };
}

// ── Abandon volontaire ────────────────────────────────────────
function actionForfeit(body) {
  const gameId = (body.gameId || '').toUpperCase().trim();
  const token  = body.token || '';
  const game   = getGame(gameId);
  const side   = sideFromToken(game, token);
  ...
  if (!side) throw new Error('Token invalide.');
  if (game.gameStatus !== 'playing') throw new Error('La partie est déjà terminée.');

  declareAbandonment(game, side, 'forfeit');
  return { ok: true, state: publicState(game, side) };
}
...
function actionList() {
  const waiting = Object.values(games)
    ...
    .map(g => ({ gameId: g.id, name: g.players.south.name, created: g.created }));
...
const DISCONNECT_TIMEOUT = 15000; // 15 secondes sans ping = déconnecté

function checkAbandonment(game) {
  if (game.gameStatus !== 'playing' || game.status !== 'playing') return;

  const now = Date.now();
  for (const side of ['north', 'south']) {
    const player = game.players[side];
    if (!player) continue;
    const timedOut = (now - player.ping) > DISCONNECT_TIMEOUT;
    if (timedOut && player.connected) {
      player.connected = false;
      // Démarrer un timer de grâce de 15 secondes supplémentaires
      player.disconnectedAt = now;
    }
    // Si déconnecté depuis plus de 15 secondes → abandon
    if (!player.connected && player.disconnectedAt && (now - player.disconnectedAt) > DISCONNECT_TIMEOUT) {
      declareAbandonment(game, side, 'timeout');
      return;
    }
  }
}

function declareAbandonment(game, loserSide, reason) {
  const winnerSide = loserSide === 'north' ? 'south' : 'north';
  const loserName  = game.players[loserSide]?.name || loserSide;
  const winnerName = game.players[winnerSide]?.name || winnerSide;

  game.gameStatus = 'ended';
  game.status     = 'ended';
  game.winner     = winnerSide;
  game.forfeit    = { loser: loserSide, reason };

  const msg = reason === 'timeout'
    ? `⚠️ ${loserName} s'est déconnecté. ${winnerName} gagne par abandon !`
    : `🏳️ ${loserName} a abandonné. ${winnerName} gagne par abandon !`;
...
function publicState(game, mySide) {
  ...
  return {
    ...
    mySide,
    players: {
      north: game.players.north ? {
        name:      game.players.north.name,
        connected: game.players.north.connected && (now - game.players.north.ping) < DISCONNECT_TIMEOUT
      } : null,
      south: {
        name:      game.players.south.name,
        connected: game.players.south.connected && (now - game.players.south.ping) < DISCONNECT_TIMEOUT
      }
    },
